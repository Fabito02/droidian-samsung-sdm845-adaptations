...
lxc-attach -n android -- pkill -9 -f "composer@2.3-service" 2>/dev/null || true
sleep 0.8
...
lxc-attach -n android -- setprop ctl.restart vendor.hwcomposer-2-3
sleep 1.5

# Final check (only once, to avoid loop overhead)
if lxc-attach -n android -- pgrep -f "composer@2.3-service" >/dev/null; then
    exit 0
fi
